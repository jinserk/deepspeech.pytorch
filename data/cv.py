...
def _download_and_preprocess_data(target_dir, work_dir):
    ...
    _maybe_convert_sets(target_dir, ARCHIVE_DIR_NAME, work_dir)
...
def _maybe_convert_sets(target_dir, extracted_data, work_dir):
    extracted_dir = path.join(target_dir, extracted_data)
    for source_csv in glob(path.join(extracted_dir, '*.csv')):
        _maybe_convert_set(extracted_dir, source_csv, path.join(work_dir, os.path.split(source_csv)[-1]), work_dir)

def _maybe_convert_set(extracted_dir, source_csv, target_csv, work_dir):
    print()
    ...
    def one_sample(sample):
        ...
        mp3_filepath = path.join(extracted_dir, mp3_filename)
        # Storing wav files next to the mp3 ones - just with a different suffix
        wav_filepath = path.join(work_dir, path.splitext(mp3_filename)[0] + ".wav")
        txt_filepath = path.join(work_dir, path.splitext(mp3_filename)[0] + ".txt")
        os.makedirs(os.path.dirname(wav_filepath), exist_ok=True)
        _maybe_convert_wav(mp3_filepath, wav_filepath)
        with open(txt_filepath, 'w') as txt_file:
            txt_file.write(sample[1])
        ...
        with lock:
            if int(frames/SAMPLE_RATE*1000/10/2) < len(str(sample[1])):
                ...
                rows.append((wav_filepath, frames/SAMPLE_RATE, txt_filepath))
...
if __name__ == "__main__":
    # argv[1]: dir to download and untar
    # argv[2]: dir to convert wav and txt
